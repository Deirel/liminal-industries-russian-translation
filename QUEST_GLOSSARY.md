...
на установленные сборки Liminal Industries для Minecraft 1.20.1
...
2. Итоговое имя объекта при `ru_ru`: языковые ресурсы этого мода, затем
   ресурсные переопределения сборки и `ru_ru.json` других установленных модов.
...
теперь локализованы ресурсами мода. Например, `Unstable Compound` называется
«Нестабильное соединение» и так же должно называться в квестах.

### Как добавлять новые термины

1. Найти строку квеста и соответствующий quest/task в SNBT.
2. Выписать точный item/block ID.
3. Проверить каталог `item-translation-work/` и языковые ресурсы мода.
