...
5. Проверьте `<mod_id>` в пути и в ключах. Сборщик сам найдёт ошибки JSON и
   повторяющиеся ключи.
6. Если версия Minecraft использует другой формат ресурс-пака, обновите
   `pack_format` в `src/resourcepack/pack.mcmeta`.

## Версии и проверка

1. Обновите `DEFAULT_VERSION` в `scripts/build_packages.py`.
2. Исправьте версии и имена архивов в `README.md` и `docs/PACKAGING.md`.
3. Соберите архивы:
...
4. Установите ресурс-пак и конфиги квестов в чистый профиль. Проверьте русский
   язык, все главы квестов и названия предметов.
5. Установите конфиги квестов на тестовый сервер. Подключитесь с установленным
   ресурс-паком. Откройте все главы и проверьте, что сервер отдаёт перевод.
...
Forge-мод не хранит отдельную копию перевода: сборка из `mod/` напрямую
подключает `src/resourcepack` и `src/quests`. После изменения этих каталогов
пересобирать или вручную синхронизировать `mod/src/main/resources` не нужно.
